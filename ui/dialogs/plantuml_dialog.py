...
from core.markdown_utils import PLANTUML_TEMPLATES, plantuml_snippet


class PlantUMLDialog(QDialog):
    """PlantUML 图表创建对话框。"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("插入 PlantUML 图表")
    ...
    def _setup_ui(self) -> None:
        ...
        self._type_combo.addItems(list(PLANTUML_TEMPLATES.keys()))
        self._type_combo.currentTextChanged.connect(self._on_type_changed)
        type_layout.addWidget(self._type_combo)
        layout.addWidget(type_group)

        code_group = QGroupBox("PlantUML 代码")
    ...
    def _on_type_changed(self, diagram_type: str) -> None:
        if diagram_type in PLANTUML_TEMPLATES:
            self._code_edit.setPlainText(PLANTUML_TEMPLATES[diagram_type])

    def _on_insert(self) -> None:
        dtype = self._type_combo.currentText()
        code = self._code_edit.toPlainText().strip()
        self._snippet = plantuml_snippet(dtype, code)
        self.accept()
